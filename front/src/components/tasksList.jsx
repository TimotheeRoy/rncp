...
function TasksList() {
	...
	const toggleTaskCompletion = async (id) => {
		try {
			const taskToUpdate = [...tasks.completed, ...tasks.not_completed].find(
				(task) => task.id === id,
			);
			...
			if (response.ok) {
				setTasks((prevTasks) => {
					...
					const updatedNotCompletedTasks = prevTasks.not_completed.filter(
						(task) => task.id !== id,
					);
					...
					tasks.not_completed.map((task) => (
						<li key={task.id} className="task">
							<input
								...
								alt="checkbox"
							/>
							<label for="uncomplete-checkbox">{task.title}</label>
							<svg
								width="32px"
								height="32px"
								viewBox="0 0 24 24"
